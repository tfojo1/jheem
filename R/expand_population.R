                              ...
                              non.conforming.error=NULL)
{
    ...
    #KEEPING FOR NOW JUST IN CASE I TURN UP A BUG - 1/27/20

    #set up indexing calcs for source and target
    source.dims = dim(source.population)
    cum.src.dim = c(1, cumprod(source.dims)[-length(source.dims)])

    target.dims = sapply(target.dim.names, length)
    cum.target.dim = c(1, cumprod(target.dims)[-length(target.dims)])

    #create mapping of flattened index to multi-dim index in target
    N.target = prod(target.dims)
    target.indices = sapply(1:length(target.dims), function(d){
        rv = ceiling(1:N.target / cum.target.dim[d]) %% target.dims[d]
        rv[rv==0] = target.dims[d]
        rv
    })
    dim(target.indices) = c(N.target, length(target.dims))

    #map from source to target
    rv = sapply(1:N.target, function(i){
        source.indices.for.i = target.indices[i,source.to.target.dims]
        source.index = 1 + sum((source.indices.for.i-1) * cum.src.dim)
        source.population[source.index]
    })

    #hydrate into array and return
    dim(rv) = sapply(target.dim.names, length)
    dimnames(rv) = target.dim.names
...
#cppFunction("NumericVector fast_expand_population(NumericVector src,
...
#    int src_dim_index;
#
#    int num_before_in_target[target_dims.length()] = {1};
...
#    int num_before_in_src[n_src_dims] = {1};
